...
def get_info():
    slack_name = flask.request.args.get('slack_name')
    track = flask.request.args.get('track')

    tracks = ["vMarketing","design","frontend","mobile", "backend"]
    if track not in tracks:
        return jsonify({"error": "Invalid track selection"}), 400
    current_utc_time = datetime.datetime.utcnow(pytz.utc)
    ...
    json_response = {
        ...
        'day_of_week': datetime.datime.now().strftime("%A"),
